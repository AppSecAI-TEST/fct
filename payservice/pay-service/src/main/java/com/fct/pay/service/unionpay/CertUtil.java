...
//	private static X509Certificate encryptTrackCert = null;
	...
	public static void init() {
		addProvider();
		if (SDKConstants.TRUE_STRING.equals(SDKConfig
				.getSingleMode())) {
			// 单证书模式,初始化配置文件中的签名证书
			initSignCert();
		}
	...
	private static void initEncryptCert() {
		...
		if (!SDKUtil.isEmpty(SDKConfig.getEncryptCertPath())) {
...
//		}
	...
	private static void initTrackKey() {
		if (!SDKUtil.isEmpty(SDKConfig.getEncryptTrackKeyModulus())
				&& !SDKUtil.isEmpty(SDKConfig.getEncryptTrackKeyExponent())) {
			encryptTrackKey = SecureUtil.getPublicKey(SDKConfig.getEncryptTrackKeyModulus(),
